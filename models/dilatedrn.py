...
def Atlas_DRN(model_name, pretrained=False, drop_rate=0., num_channels=4):
    if model_name == 'drn-d-105':
        print("Using DRN d-105")
        if pretrained:
            print('Loading weights...')
            model = drn_d_105(pretrained=True)
        else:
            model = drn_d_105(pretrained=False)
        ...
        if num_channels == 4:
            nconv = nn.Conv2d(4, 16, kernel_size=(7, 7), stride=(1, 1), padding=(3, 3), bias=False)

            if pretrained:
                nconv.weight.data[:,:3,:,:] = model.layer0[0].weight.data.clone()
                nconv.weight.data[:,3,:,:] = model.layer0[0].weight.data[:,1,:,:].clone()
            
            model.layer0[0] = nconv
        
        model.fc = nn.Conv2d(512, 28, kernel_size=(1, 1), stride=(1, 1))

    if model_name == 'drn-d-54':
        print("Using DRN d-54")
        if pretrained:
            print('Loading weights...')
            model = drn_d_54(pretrained=True)
        else:
